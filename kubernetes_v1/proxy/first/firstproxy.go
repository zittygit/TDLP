...
func startServer(configFile *string) {
	fmt.Println("starting first proxy server ...")
	...
	if pidFileName == "" {
		pidFileName = "proxy/first/firstproxy.pid"
	}
	...
	if err != nil {
		...
		if err = syscall.Kill(pid, 0); err == nil {
			fmt.Println("first proxy server with pid " + strconv.Itoa(pid) + " is running, stop it first")
			os.Exit(1)
		}
	}
	cmd := exec.Command("bin/firstproxyd", os.Args[1:]...)
	err = cmd.Start()
	if err != nil {
		fmt.Println("failed to start first proxy server")
		os.Exit(1)
	}
	fmt.Println("first proxy server started with pid " + strconv.Itoa(cmd.Process.Pid))
...
func stopServer(configFile *string) {
	fmt.Println("stoping first proxy server ...")
	myConfig := new(conf.Config)
	myConfig.InitConfig(configFile)
	logFileName := myConfig.Read("logfile")
	if logFileName == "" {
	...
	if err != nil {
		...
		if err = syscall.Kill(pid, 0); err == nil {
			if err = syscall.Kill(pid, syscall.SIGTERM); err == nil {
				...
				fmt.Println("first proxy server with pid " + strconv.Itoa(pid) + " stopped")
			} else {
				fmt.Println("failed to stop first proxy server with pid " + strconv.Itoa(pid))
				os.Exit(1)
			}
		} else {
			fmt.Println("first proxy server with pid " + strconv.Itoa(pid) + " is not running")
		}
	}
}
