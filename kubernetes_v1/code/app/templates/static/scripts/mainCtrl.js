...
function createAppData(app_data){
            ...
            for (var i = 0; i<apps.length; i++) {
                ...
                app_table_data.push({
                    ...
                    "image": app.appMeta.image,
                    // "manage": '<button class="btn btn-danger btn-sm delete_app" >Delete</button>',
                    // "detail": '<button class="btn btn-default btn-sm">Detail</button>'
                    "manage": '<input type="button" title="Delete App" class="delete_app" >',
                    "detail": '<input type="button" title="More Details" class="detail_app">',
                    "pod_name": pod_name
                });
            }
...
function createALLothersAppData(app_data){
            ...
            for (var i = 0; i<apps.length; i++) {
                ...
                app_table_data.push({
                    ...
                    "gpu_mode": app.appMeta.gpu_mode === 1? 'Dedicated': 'Shared',
                    "detail": '<input type="button" title="More Details" class="detail_other">',
                    "pod_name": pod_name
                });
            }
            return app_table_data
...
var app = angular.module('app', [])
    ...
    $scope.single_app_info={};
    $scope.app_table_data = [];
    $scope.app_table_all = [];
    ...
    function setAppFrame() {
        var cpu_src = "../grafana_data/dashboard-solo/db/pods?var-namespace=default&var-podname="+
            $scope.single_app_info.pod_name+"&panelId=6&theme=light&width=400&height=300";
        var mem_src = "../grafana_data/dashboard-solo/db/pods?var-namespace=default&var-podname="+
            $scope.single_app_info.pod_name+"&panelId=7&theme=light";
        var net_src = "../grafana_data/dashboard-solo/db/pods?var-namespace=default&var-podname="+
            $scope.single_app_info.pod_name+"&panelId=8&theme=light";
        var file_src = "../grafana_data/dashboard-solo/db/pods?var-namespace=default&var-podname="+
            $scope.single_app_info.pod_name+"&panelId=9&theme=light";
        $("#app_cpu").attr('src',cpu_src);
        $("#app_mem").attr('src',mem_src);
        $("#app_net").attr('src',net_src);
        $("#app_file").attr('src',file_src);
        $('#appdetail-modal').modal('show');
    }
    ...
    function updateAppTable(){
        if ($("#app-table").length > 0){
            $.get( "../app_data/api/v1/pod/default", function( app_data ) {
                ...
                if ($scope.app_table_data.length === 0) {
                ...
                $('.detail_app').click(function(e){
                    var index = e.target.parentElement.parentElement.rowIndex -1;
                    $scope.single_app_info = $scope.app_table_data[index];
                    $scope.single_app_info.app_name = $scope.single_app_info.app_name.slice(3,-4);
                    setAppFrame();
                });
            });
        }
        ...
        if ($("#allothers-app-table").length > 0){
            $.get( "../app_data_allothers/api/v1/pod/default", function( app_data ) {
                ...
                $('.detail_other').click(function(e){
                    var index = e.target.parentElement.parentElement.rowIndex -1;
                    $scope.single_app_info = $scope.app_table_all[index];
                    setAppFrame();
                });
    ...
    $('#appdetail-modal').on('show.bs.modal', function() {
        $scope.$apply();
    });
    ...
    if ( $( "#system_resource" ).length > 0 ) {
        ...
        time_from = time_to - 15*60*1000;
        cpu_solo_src = "../grafana_data/dashboard-solo/db/overall?from="+time_from+"&to="+time_to+"&panelId=1&theme=light&width=150&height=150";
        mem_solo_src = "../grafana_data/dashboard-solo/db/overall?from="+time_from+"&to="+time_to+"&panelId=2&theme=light&width=150&height=150";
    ...
    $('#workspace').load(function(){
        if($('#app-tab').children().length <= 1) {
            ...
            $('.full-screen-btn').css('display','block').click(function(){
